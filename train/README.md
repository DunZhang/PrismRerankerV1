...
总损失公式为 `L_total = α · L_rank + β · L_listwise + γ · L_pointwise`，以下五个参数控制三个 loss 的权重和内部行为。

#### 三个权重参数

- **`alpha_rank`**（默认 `1.0`）：`L_rank` 的权重，控制"正例必须排在所有负例前面"这个基本约束的强度。
  - 调大 → 模型更拼命确保正例排第一，适合正例排序准确率不够的情况。
  - 调小 → 正例排序约束放松，模型更依赖 listwise 和 pointwise 来引导。

- **`beta_list`**（默认 `1.0`）：`L_listwise` 的权重，控制"学生的完整排序分布要逼近教师"的强度。这是传递 dark knowledge（负例之间的相对难度）的主要通道。
  - 调大 → 模型更努力学习负例之间的精细排序（比如教师觉得 0.53 比 0.19 更相关）。
  - 调小 → 不太关心负例之间的相对顺序。

- **`gamma_point`**（默认 `0.5`）：`L_pointwise` 的权重，控制"学生输出的绝对分数要逼近教师分数"的强度。默认 0.5 是因为它是辅助目标——分数校准重要但不如排序本身。
  - 调大 → 绝对分数更准，适合需要做阈值截断的场景（如"只保留分数 > 0.5 的文档"）。
  - 调小 → 模型更自由地学排序，不被绝对分数约束。

默认 `1.0 / 1.0 / 0.5` 的逻辑：rank 和 listwise 是核心排序目标，同等重要；pointwise 是校准辅助，权重减半。

#### `temperature`（默认 `2.0`）

只影响 `L_listwise`。在算 KL 散度之前，把教师和学生的 logits 都除以温度 T 再做 softmax，用来控制分布的"软硬程度"：

| T 值 | 分布特征 | 效果 |
|------|---------|------|
| 1.0 | 尖锐，最高分文档占绝对主导 | 只学"谁是第一"，几乎丢失中间排序信息 |
| **2.0** | **适中，保留排序差异又不过度平滑** | **默认值，平衡学习效果** |
| 5.0+ | 很平，接近均匀分布 | 排序差异被抹平，学不到有意义的信息 |

最终乘以 T² 是 Hinton 蒸馏的标准做法——温度升高会让 KL 梯度变小，T² 补偿回来保持梯度幅度稳定。

- 调大 → 分布更平滑，模型学到更多负例之间的微妙差异，但太大（>5）会丢失信号。
- 调小 → 分布更尖锐，训练更"激进"但可能不稳定。
- 一般在 1.0 ~ 4.0 之间尝试，默认 2.0 是经验上比较安全的起点。

**通过 wandb 判断温度是否合适**：观察 `loss_listwise` 曲线——
- 一直很小但 MRR 不涨 → 温度太高，分布太平，梯度无意义，**调小**
- 波动剧烈、训练不稳 → 温度太低，梯度太大，**调大**

#### `hard_neg_scale`（默认 `5.0`）

只影响 `L_rank`。用教师的正负分差（margin）来衡量每个负例的难度，然后对负例做加权，公式为：

```
margin = teacher_pos_score - teacher_neg_score
difficulty = 1 / (1 + margin × hard_neg_scale)
weight = softmax(difficulty)  // 对 7 个负例归一化
```

**直觉**：margin 越小说明教师也觉得这个负例和正例很像（硬负例），应该给更多训练权重。`hard_neg_scale` 控制这种区分度的放大倍数。

**举例**：假设正例教师分 0.9，三个负例教师分分别是 0.85、0.5、0.1：

| 负例分数 | margin | difficulty (scale=5) | difficulty (scale=1) |
|---------|--------|---------------------|---------------------|
| 0.85 | 0.05 | 1/(1+0.25) = **0.80** | 1/(1+0.05) = 0.95 |
| 0.50 | 0.40 | 1/(1+2.0) = **0.33** | 1/(1+0.40) = 0.71 |
| 0.10 | 0.80 | 1/(1+4.0) = **0.20** | 1/(1+0.80) = 0.56 |

scale=5 时，硬负例（0.85）权重远高于简单负例（0.10），比值 0.80:0.20 = 4:1；scale=1 时差距小得多，0.95:0.56 ≈ 1.7:1。

- 调大 → 训练更聚焦于教师也觉得难区分的负例，适合数据中有很多"似是而非"的负例时。
- 调小 → 更均匀地对待所有负例，适合负例质量参差不齐、不想过度依赖少数硬负例时。
- 调到 0 → 完全均匀权重，等价于标准 InfoNCE。

#### 推荐调参顺序

1. **先跑默认值**，观察 dev MRR 和三个分 loss 的曲线。
2. 如果 `loss_rank` 很高迟迟不降 → 试着调大 `alpha_rank` 到 2.0。
3. 如果排序粗糙（MRR 可以但 NDCG@5 不行）→ 调大 `beta_list` 到 2.0，或适当提高 `temperature` 到 3.0。
4. 如果需要用分数做阈值截断且分数偏移严重 → 调大 `gamma_point` 到 1.0。
5. `hard_neg_scale` 一般不需要动，除非数据中硬负例特别多或特别少。
6. `temperature` 通常在 1.0 ~ 4.0 范围内网格搜索，影响比较直观。
