...
_hfds.logging.set_verbosity_info()
_hfds.enable_progress_bars()
...
def mean_score(scores: list[float]) -> float:
    """Mean of a list of scores; returns 0.0 if empty."""
...
def build_corpus_texts(
    ...
    cols = ["_id", "title", "text"] if has_title else ["_id", "text"]
    for row in tqdm(
        corpus_ds.select_columns(cols),
        total=n,
        desc="  Building corpus texts",
        unit="doc",
    ):
...
def process_dataset(
    ...
    # --- Load data ---
    print(f"  Pre-downloading {dataset_name} repo snapshot...", flush=True)
    ...
    print(f"  Loading corpus for {short_name}...", flush=True)
    ...
    # --- Save qrels JSON (keyed by query_text for eval_topk) ---
    text_qrels = _rekey_qrels_by_text(
        qrels, filtered_query_ids, filtered_query_texts
    )
    qrels_path = output_path.parent / make_qrels_filename(dataset_name)
    with open(qrels_path, "w", encoding="utf-8") as fw:
        json.dump(text_qrels, fw, ensure_ascii=False)
    print(f"  Saved qrels -> {qrels_path.name}")
...
def make_qrels_filename(dataset_name: str) -> str:
    """Convert 'mteb/trec-covid' to 'mteb__trec-covid_qrels.json'."""
    safe_name = dataset_name.replace("/", "__")
    return f"{safe_name}_qrels.json"


def _rekey_qrels_by_text(
    qrels: dict[str, dict[str, int]],
    query_ids: list[str],
    query_texts: list[str],
) -> dict[str, dict[str, int]]:
    """Re-key qrels from query_id to query_text for local storage."""
    id_to_text = dict(zip(query_ids, query_texts))
    return {
        id_to_text[qid]: rels
...
def main() -> None:
    ...
    for di, dataset_name in enumerate(args.datasets, 1):
        ...
        qrels_filename = make_qrels_filename(dataset_name)
        qrels_path = output_dir / qrels_filename
        if output_path.exists() and qrels_path.exists():
