...
def _load_vote_counts(save_path: Path) -> dict[str, tuple[int, int]]:
    ...
    Deduplicates by (pair_hash, model_name) so that duplicate rows for the
    same model+pair only count as one vote.
    """
    counts: dict[str, list[int]] = {}  # pair_hash -> [yes, no]
    seen: set[tuple[str, str]] = set()  # (pair_hash, model_name)
    if not save_path.exists() or save_path.stat().st_size == 0:
        return {}
    with open(save_path, encoding="utf-8") as f:
        for line in f:
...
_MODEL_EXTRA_KWARGS: dict[str, dict[str, Any]] = {
    ...
    "openrouter/google/gemini-3.1-flash-lite-preview": {
        "max_tokens": 5,  # lite model has no thinking; 1~4 returns empty
    },
