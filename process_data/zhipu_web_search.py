"""Augment KaLM JSONL rows with Zhipu AI web search results.

Usage:
uv run python -m process_data.zhipu_web_search \
...
- Uses ``query`` to call Zhipu AI web search.
- Adds ``extra.original_zhipu_result`` to each written row.
...
- Rotates across all ``ZHIPU_API_KEY_{idx}`` keys from ``.env``.
...
log = logging.getLogger("zhipu_web_search")
...
KEY_LIMIT = 5000
KEY_RPM = 60
ZHIPU_ENV_RE = re.compile(r"^ZHIPU_API_KEY_(\d+)$")
...
class ZhipuKeyAllocator:
    ...
    def __init__(
        ...
        if not keys:
            raise ValueError("At least one Zhipu key is required.")
        self.keys = keys
    ...
    def reserve(self) -> ZhipuKey | None:
...
def _load_zhipu_keys() -> list[ZhipuKey]:
    keys: list[ZhipuKey] = []
    for env_name, env_value in os.environ.items():
        match = ZHIPU_ENV_RE.fullmatch(env_name)
        if not match or not env_value:
            continue
        keys.append(
            ZhipuKey(index=int(match.group(1)), name=env_name, value=env_value.strip())
        )
    keys.sort(key=lambda item: item.index)
    return keys


def _new_allocator(keys: list[ZhipuKey]) -> ZhipuKeyAllocator:
    """Create a fresh allocator with zero usage (per-run counting)."""
    return ZhipuKeyAllocator(
        keys=keys,
        key_usage={key.name: 0 for key in keys},
    )


def _search_with_zhipu(api_key: str, query: str) -> list[dict[str, Any]]:
    try:
        from zai import ZhipuAiClient
    except ImportError as exc:  # pragma: no cover - runtime environment dependent
        raise RuntimeError(
            "Missing dependency: zai. Install it with `uv add zai`."
        ) from exc
    ...
    return [item.model_dump(mode="json") for item in response.search_result]
...
def _is_usage_limit_error(exc: Exception) -> bool:
    ...
    has_quota_hint = any(
        token in message
        for token in ("usage limit", "quota", "credits", "credit balance", "balance")
    )
...
def _augment_row(
    ...
    extra["original_zhipu_result"] = response
    out_row["extra"] = extra
    return out_row
...
def _process_batch(
    ...
    allocator: ZhipuKeyAllocator,
    max_workers: int,
    fout,  # noqa: ANN001
    done_hashes: set[str],
) -> tuple[int, int, bool]:
    reservations: list[tuple[str, dict[str, Any], ZhipuKey]] = []
    exhausted = False
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as pool:
        future_to_request = {
            pool.submit(_search_with_zhipu, key.value, row["query"]): (row_hash, key)
            ...
            try:
                ...
                if _is_usage_limit_error(exc):
                    if allocator.disable_for_current_run(key.name):
                        log.warning(
                            "Zhipu quota exhausted for key=%s; disabling it for the "
                            "rest of this run.",
                            key.name,
                        )
    ...
    for row_hash, row, key in reservations:
        ...
        if isinstance(result, Exception):
            failed += 1
            log.warning(
                "Zhipu search failed for key=%s query=%r: %s: %s",
                key.name,
                row.get("query", "")[:80],
...
def process(
    ...
    keys = _load_zhipu_keys()
    if not keys:
        log.error("No ZHIPU_API_KEY_{idx} keys found in environment or .env.")
        sys.exit(1)
    ...
    if remaining_capacity == 0:
        log.warning(
            "No remaining Zhipu quota. Add more keys or reset quota, then rerun."
        )
        return
    ...
    if exhausted:
        log.warning(
            "Stopped because all currently usable Zhipu keys are exhausted or have "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Augment KaLM JSONL with Zhipu AI web search results."
    )
    parser.add_argument(
        "--read_path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Max parallel Zhipu requests per batch (default: 16).",
    )
    parser.add_argument(
        "--env_file",
        type=Path,
