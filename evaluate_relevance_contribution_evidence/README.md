...
2. **Contribution / Evidence 质检**：对 `annotated_label == pred_label == "yes"` 的行，
   **由大模型作为裁判**（默认 Kimi k2.5，思考模式开启；也支持 DeepSeek deepseek-reasoner），
   按 6 个维度（5 分制）给模型抽取的 contribution 和 evidence 打分。
   也就是说，contribution / evidence 的质量评分是通过调用大模型 API 完成的，并非规则匹配。
